...
const {
  ...
  modelStats, loading, error, registerTeam, joinTeam
} = useTeams()
...
const modelOptions = [
  { id: 'GLM', label: 'GLM', icon: '/sponsors/zhipu.png' },
  { id: 'MiniMax', label: 'MiniMax', icon: '/sponsors/minimax.ico' },
  { id: 'Kimi', label: 'Kimi', icon: '/sponsors/kimi.ico' },
]

const avatarPresets = [
  { id: 'glm', label: 'GLM', src: '/sponsors/zhipu.png' },
  { id: 'minimax', label: 'MiniMax', src: '/sponsors/minimax.ico' },
  { id: 'kimi', label: 'Kimi', src: '/sponsors/kimi.ico' },
]
...
function teamVibe(count: number) {
  if (count === 1) return { label: 'Fly Solo', color: 'text-accent-yellow' }
  if (count === 2) return { label: 'Dynamic Duo', color: 'text-accent-cyan' }
  return { label: 'Three Musketeers', color: 'text-accent-red' }
}
...
const inputClass = 'w-full px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/30 focus:border-accent-red/50 focus:outline-none transition-colors text-sm'
</script>
...
<template>
  <section id="teams" class="relative py-32 bg-bg-secondary overflow-hidden">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="max-w-2xl mx-auto mb-12 reveal">
        <div class="flex justify-between text-sm mb-3">
          <span class="text-text-secondary">
            <span class="text-white font-bold">{{ teams.length }}</span> teams ·
            <span class="text-white font-bold">{{ totalMembers }}</span> participants
          </span>
          <span class="text-text-secondary">
            <span class="text-accent-yellow font-bold">{{ spotsLeft }}</span> spots left
          </span>
        </div>
        <div class="w-full h-2 bg-white/5 rounded-full overflow-hidden">
          <div class="h-full bg-accent-red rounded-full transition-all duration-1000" :style="{ width: `${progress}%` }"></div>
        </div>
        <div class="flex justify-center gap-6 mt-6">
          <div v-for="(count, model) in modelStats" :key="model" class="flex items-center gap-2">
            <img v-if="getModelIcon(model as string)" :src="getModelIcon(model as string)" class="w-5 h-5 rounded" />
            <span class="text-sm text-text-secondary">
              <span class="text-white font-semibold">{{ model }}</span>
              <span class="text-accent-yellow ml-1">{{ count }}</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="text-center mb-12 reveal">
        <button @click="openCreateModal" :disabled="isFull" class="px-8 py-4 bg-gradient-to-r from-accent-red to-accent-red-dark text-white font-bold text-lg rounded-full hover:shadow-xl hover:shadow-accent-red/30 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div v-for="team in teams" :key="team.id" @click="canJoin(team) && openJoinModal(team)" class="glass-card p-5 transition-all group" :class="canJoin(team) ? 'cursor-pointer hover:scale-[1.03] hover:border-accent-cyan/40' : ''">
          <div class="flex items-center gap-3 mb-2">
            <img :src="team.avatar || '/default-avatar.svg'" class="w-10 h-10 rounded-full shrink-0 object-cover border border-white/10" />
            <div class="min-w-0">
              <h3 class="font-bold text-white text-sm truncate group-hover:text-accent-red transition-colors">{{ team.name }}</h3>
              <p class="text-text-secondary text-xs truncate">{{ trackShort(team.track) }}</p>
            </div>
          </div>
          <div class="flex items-center gap-2 mb-1">
            <span class="text-[10px] px-2 py-0.5 rounded-full bg-white/5 font-semibold" :class="teamVibe(team.members.length).color">{{ teamVibe(team.members.length).label }}</span>
            <span v-if="canJoin(team)" class="text-[10px] px-1.5 py-0.5 rounded bg-accent-cyan/10 text-accent-cyan">+{{ 3 - team.members.length }} open</span>
          </div>

          <div class="space-y-1.5">
            <div v-for="member in team.members" :key="member.name" class="flex items-center gap-2">
              <div class="w-1.5 h-1.5 rounded-full bg-accent-cyan shrink-0"></div>
              <span class="text-xs text-white/80 truncate">{{ member.name }}</span>
              <a v-if="member.githubId" :href="(member.githubId.includes('gitlab') ? 'https://gitlab.com/' : 'https://github.com/') + member.githubId.replace(/^@/, '')" target="_blank" @click.stop class="text-[10px] text-text-secondary hover:text-accent-cyan transition-colors truncate">@{{ member.githubId.replace(/^@/, '') }}</a>
            </div>
          </div>

          <a v-if="team.githubRepo" :href="team.githubRepo" target="_blank" @click.stop class="inline-flex items-center gap-1 mt-2 text-[11px] text-text-secondary hover:text-accent-cyan transition-colors">
      ...
      <Transition enter-active-class="transition-opacity duration-200" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition-opacity duration-150" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-lg glass-card p-8 max-h-[90vh] overflow-y-auto border-accent-red/20">
            <button @click="showModal = false" class="absolute top-4 right-4 text-text-secondary hover:text-white">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>

            <!-- CREATE MODE -->
            <template v-if="modalMode === 'create'">
              <h3 class="text-2xl font-bold text-white mb-6">Register Your Team</h3>
              <div v-if="error" class="mb-4 p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-sm">{{ error }}</div>
              ...
              <form @submit.prevent="submitCreate" class="space-y-5">
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <div class="w-14 h-14 rounded-full border-2 border-white/10 overflow-hidden shrink-0">
                      <img :src="teamAvatar || defaultAvatar()" class="w-full h-full object-cover" />
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                      <button v-for="preset in avatarPresets" :key="preset.id" type="button" @click="teamAvatar = preset.src" class="w-9 h-9 rounded-full border-2 overflow-hidden transition-all" :class="teamAvatar === preset.src ? 'border-accent-red scale-110' : 'border-white/10 hover:border-white/30'">
                        <img :src="preset.src" class="w-full h-full object-cover" />
                      </button>
                      <label class="w-9 h-9 rounded-full border-2 border-dashed border-white/20 hover:border-white/40 flex items-center justify-center cursor-pointer transition-all overflow-hidden" :class="teamAvatar && !avatarPresets.some(p => p.src === teamAvatar) ? 'border-accent-red' : ''">
                ...
                <div>
                  ...
                  <div class="flex gap-3">
                    <button v-for="n in 3" :key="n" type="button" @click="memberCount = n" class="flex-1 py-2.5 rounded-lg border font-semibold transition-all flex flex-col items-center gap-0.5" :class="memberCount === n ? 'bg-accent-red/20 border-accent-red/50 text-white' : 'border-white/10 text-text-secondary hover:border-white/30'">
                      <span>{{ n }}</span>
                ...
                <div>
                  ...
                  <div class="space-y-3">
                    <div v-for="(_, i) in members.slice(0, memberCount)" :key="i" class="p-3 rounded-lg bg-white/[0.02] border border-white/5">
                ...
                <div>
                  ...
                  <div class="flex gap-3">
                    <button v-for="model in modelOptions" :key="model.id" type="button" @click="selectModel(model.id)" class="flex-1 flex items-center justify-center gap-2 py-3 rounded-lg border transition-all" :class="selectedModel === model.id ? 'bg-accent-red/20 border-accent-red/50 text-white' : 'border-white/10 text-text-secondary hover:border-white/30'">
                ...
                <button type="submit" :disabled="loading" class="w-full py-4 bg-gradient-to-r from-accent-red to-accent-red-dark text-white font-bold rounded-full hover:shadow-lg hover:shadow-accent-red/30 transition-all disabled:opacity-50">
            ...
            <template v-else-if="modalMode === 'join' && joiningTeam">
              <h3 class="text-2xl font-bold text-white mb-2">Join Team</h3>
              <p class="text-accent-red font-semibold mb-1">{{ joiningTeam.name }}</p>
